...
struct AgentToolLoopTests {
    ...
    @Test func toolResultReturnsToSecondModelStepAsStructuredHistory() async throws {
        ...
        #expect(requests[0].tools.map(\.id.rawValue) == ["shell", "read_file", "write_file", "edit_file", "apply_patch", "list_directory", "glob", "grep", "web_search", "web_fetch", "search_tools", "load_tool", "context_recall", "run_background_command", "manage_background_command"].sorted())
