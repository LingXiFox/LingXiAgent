...
public struct LingXiClient: Sendable {
    ...
    public func coreInfo() async throws -> CoreInfo {
        guard case let .info(info) = try await send(.getInfo) else {
    ...
    public func coreState() async throws -> CoreState {
        guard case let .state(state) = try await send(.getState) else {
    ...
    public func ping() async throws {
        guard case .pong = try await send(.ping) else {
    ...
    public func providerStatus() async throws -> ProviderStatus {
        guard case let .providerStatus(status) = try await send(.getProviderStatus) else {
    ...
    public func createSession() async throws -> SessionID {
        guard case let .sessionCreated(info) = try await send(.createSession) else {
    ...
    public func sessions() async throws -> [SessionInfo] {
        guard case let .sessionList(infos) = try await send(.listSessions) else {
    ...
    public func replyQuestion(_ reply: QuestionReply) async throws {
        guard case let .questionReplyAccepted(questionID) = try await send(.replyQuestion(reply)), questionID == reply.questionID else {
    ...
    public func context(_ sessionID: SessionID) async throws -> ContextDebugSnapshot? {
        guard case let .context(snapshot) = try await send(.getContext(sessionID: sessionID)) else {
    ...
    public func performance(_ sessionID: SessionID) async throws -> TurnPerformanceReport? {
        guard case let .performance(report) = try await send(.getPerformance(sessionID: sessionID)) else {
    ...
    public func permissionConfiguration() async throws -> PermissionConfiguration {
        guard case let .permissionConfiguration(configuration) = try await send(.getPermissionConfiguration) else {
    ...
    public func setPermissionConfiguration(_ configuration: PermissionConfiguration) async throws {
        guard case .permissionConfiguration = try await send(.setPermissionConfiguration(configuration)) else {
    ...
    public func compact(_ sessionID: SessionID) async throws -> CompactSessionResponse {
        guard case let .compactSession(response) = try await send(.compactSession(sessionID: sessionID)) else {
    ...
    public func listChildSessions(_ parentSessionID: SessionID) async throws -> [SessionInfo] {
        guard case let .childSessionList(sessions) = try await send(.listChildSessions(parentSessionID: parentSessionID)) else { throw CoreError(code: .transport, message: "listChildSessions 收到非预期响应") }
        return sessions
    }

    public func listAgentRuns(_ sessionID: SessionID) async throws -> [AgentRunInfo] {
        guard case let .agentRunList(runs) = try await send(.listAgentRuns(sessionID: sessionID)) else { throw CoreError(code: .transport, message: "listAgentRuns 收到非预期响应") }
        return runs
    }

    public func getAgentRun(_ runID: AgentRunID) async throws -> AgentRunInfo {
        guard case let .agentRun(run) = try await send(.getAgentRun(runID: runID)) else { throw CoreError(code: .transport, message: "getAgentRun 收到非预期响应") }
        return run
    }

    public func getAgentTree(_ rootSessionID: SessionID) async throws -> AgentTreeNode {
        guard case let .agentTree(tree) = try await send(.getAgentTree(rootSessionID: rootSessionID)) else { throw CoreError(code: .transport, message: "getAgentTree 收到非预期响应") }
        return tree
    }

    public func subagentResult(_ runID: AgentRunID) async throws -> SubagentResult {
        guard case let .subagentResult(result) = try await send(.getSubagentResult(runID: runID)) else { throw CoreError(code: .transport, message: "getSubagentResult 收到非预期响应") }
        return result
    }
    ...
    private func send(_ command: ClientCommand) async throws -> CoreResponse {
        let response = try await connection.send(command)
        if case let .error(error) = response { throw error }
        return response
    }
