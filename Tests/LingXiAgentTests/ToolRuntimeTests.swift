...
struct ToolRuntimeTests {
    ...
    @Test func definitionsAndRegistryAreStable() async throws {
        ...
        defer { try? FileManager.default.removeItem(at: root) }
        let runtime = try runtime(root: root)
        #expect(runtime.definitions.map(\.id.rawValue) == ["apply_patch", "browser_act", "browser_navigate", "codebase_graph", "computer_batch", "context_recall", "edit_file", "format_file", "git", "glob", "grep", "list_directory", "manage_background_command", "process", "question", "read_file", "retrieval_search", "run_background_command", "shell", "skill", "todo", "web_fetch", "web_search", "write_file"])
