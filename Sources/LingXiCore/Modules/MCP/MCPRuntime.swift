...
public enum MCPDiscoveryTools {
    public static let search = ToolDefinition(id: ToolID("search_tools"), description: "Search external MCP tool capabilities across installed and active servers. Returns tool metadata and IDs.", inputSchema: ToolInputSchema(properties: ["query": ToolInputProperty(type: .string, description: "Capability query or keyword"), "server": ToolInputProperty(type: .string, description: "Optional server alias to filter results"), "capability": ToolInputProperty(type: .string, description: "Optional capability tag"), "max_results": ToolInputProperty(type: .integer, description: "Maximum candidates", minimum: 1, maximum: 8)], required: ["query"]), capability: ToolCapability(readOnly: true))
